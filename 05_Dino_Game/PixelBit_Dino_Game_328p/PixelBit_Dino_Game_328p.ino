...
void loop() {
  ...
  if ((digitalRead(APIN_BUTTON_A) == BTN_LVL_PRESSED) 
    && (btn_a_stat == BTN_LVL_RELEASE)) {
    btn_a_stat = BTN_LVL_PRESSED;
    Serial.print(ATM_EVN_BTNA_PRE);
  } else if ((digitalRead(APIN_BUTTON_A) == BTN_LVL_RELEASE) 
    && (btn_a_stat == BTN_LVL_PRESSED)) {
    btn_a_stat = BTN_LVL_RELEASE;
    Serial.print(ATM_EVN_BTNA_REL);
  } 

  // read button b status
  if ((digitalRead(APIN_BUTTON_B) == BTN_LVL_PRESSED) 
    && (btn_b_stat == BTN_LVL_RELEASE)) {
    btn_b_stat = BTN_LVL_PRESSED;
    Serial.print(ATM_EVN_BTNB_PRE);
  } else if ((digitalRead(APIN_BUTTON_B) == BTN_LVL_RELEASE) 
    && (btn_b_stat == BTN_LVL_PRESSED)) {
    btn_b_stat = BTN_LVL_RELEASE;
    Serial.print(ATM_EVN_BTNB_REL);
  } 
}
